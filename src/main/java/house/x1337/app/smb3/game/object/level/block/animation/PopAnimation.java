 ...
 * <ul>
 *   <li>A {@link PopMotion} describing lifetime, vertical offset and texture frame per tick</li>
...
public interface PopAnimation extends GameEngineAware, GameRenderer {
    ...
    /** The function driving lifetime, vertical displacement and texture cycling. */
    PopMotion getMotion();

    WorldOffset getInitialWorldOffset();
    ...
    default void tick() {
        ...
        if (getFrameIndex() >= getMotion().durationTicks()) {
            setExpired(true);
            return;
        }
    ...
    default void positionSprite() {
        ...
        getSpriteGeometry().setLocalTranslation(getInitialWorldOffset().x(), worldY, getInitialWorldOffset().z());
    }
    ...
    default float calculateWorldY() {
        ...
        return getInitialWorldOffset().y() + (float) yOffset / TILE_SPRITE_SIZE;
    }
